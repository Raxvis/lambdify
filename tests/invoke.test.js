...
test('invoke calls local lambda function', async () => {
	const res = await lambdify.invoke(response, path.resolve(__dirname, 'fn/fooBar.handler'));
...
test('invoke handles function', async () => {
	...
	const res = await lambdify.invoke(response, fn.handler);

	expect(JSON.parse(res.body).payload).toEqual({ foo: 'bar' });
});

test('invoke throws error if bad handler', () => {
	expect(lambdify.invoke(response, {})).rejects.toThrow();
});

test('invoke return correct data', async () => {
	const res = await lambdify.invoke(
		{ body: JSON.stringify(response) },
		path.resolve(__dirname, 'fn/request.handler'),
	);
...
test('invoke handles errors', async () => {
	const res = await lambdify.invoke(response, path.resolve(__dirname, 'fn/error.handler'));
